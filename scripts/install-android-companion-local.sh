...
# Android's signing pipeline is align -> sign -> verify. Termux package `aapt`
# provides a native zipalign binary; do not sign an unaligned release APK.
...
# Fail before signing if Android's own resource parser cannot read the downloaded APK.
...
# The key uses the same password as the keystore. Omit --key-pass so apksigner
# does not consume the same one-line password source twice.
...
# Verify again after signing that Android resource/package parsing still succeeds.
