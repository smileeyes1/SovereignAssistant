...
UNSAFE = ['اكشف الأسرار', 'الغاية تبرر الوسيلة', 'نفذ الخطر دون تفويض']
CRITICAL_INHERITED = ['م★', 'عقد★', 'منع★', 'مرئي★', 'عربية★']


def validate(text: str, enforce_digest: bool = True):
    text = text.strip()
    errors = []
    ...
    if enforce_digest and sha256 != EXPECTED_SHA256:
    ...
    if not lambda_match:
        ...
        for concept in CRITICAL_INHERITED:
            if concept not in inherited:
                errors.append(f'critical concept not inherited: {concept}')
    ...
    return errors, sha256, len(paragraphs)


def main():
    text = PATH.read_text(encoding='utf-8')
    errors, sha256, paragraphs = validate(text, enforce_digest=True)
    print(f'path={PATH}')
