...
def valid_transition(record: dict[str, Any], root: Path = ROOT) -> bool:
    if record.get("approved") is not True or record.get("mode") not in {"SUPERSEDE", "RETIRE"}:
        return False
    ...
    return isinstance(tests, list) and bool(tests) and all((root / str(path)).is_file() for path in tests)


def loss_allowed(transitions: list[dict[str, Any]], target_type: str, target: str, root: Path = ROOT) -> bool:
    return any(
...
def static_violations(root: Path, policy: dict[str, Any]) -> list[Violation]:
    ...
    allowed_statuses = {"PROTECTED_ACTIVE", "PROTECTED_HISTORICAL", "SUPERSEDED", "RETIRED_SECURITY_DENY"}
    ...
    lineage = active.get("promotion_lineage", {}) if isinstance(active.get("promotion_lineage", {}), dict) else {}
...
def git_show(base_ref: str, rel: str) -> str | None:
    result = subprocess.run(["git", "show", f"{base_ref}:{rel}"], cwd=ROOT, text=True, capture_output=True, check=False)
    return result.stdout if result.returncode == 0 else None


def state_regressions(base: dict[str, Any], current: dict[str, Any], transitions: list[dict[str, Any]], root: Path = ROOT) -> list[Violation]:
    violations: list[Violation] = []
    for lost in sorted(set(base.get("proven_success", [])) - set(current.get("proven_success", []))):
        ...
        for key, value in base_lineage.items():
            if key not in current_lineage:
                ...
                if not loss_allowed(transitions, "promotion_lineage", target, root):
                    violations.append(Violation("PROMOTION_LINEAGE_REWRITTEN", key, f"{value} -> {current_lineage[key]}"))
        ...
        for key, value in base_evidence.items():
            if value == "PASS" and current_evidence.get(key) != "PASS":
                if not loss_allowed(transitions, "evidence", key, root):
                    violations.append(Violation("PASS_EVIDENCE_DEGRADED", key, f"PASS -> {current_evidence.get(key)!r}"))
    return violations
...
def diff_violations(root: Path, policy: dict[str, Any], base_ref: str) -> list[Violation]:
    ...
    for status, old, new in entries:
        ...
        if (status.startswith("D") or status.startswith("R")) and old.startswith(prefixes):
    ...
    for rel in api_surfaces:
        ...
        if before is None or not path.is_file():
            continue
        removed = public_python_symbols(before) - public_python_symbols(path.read_text(encoding="utf-8"))
    ...
    for rel, status in changed.items():
        ...
        if before is None or not path.is_file():
            continue
        removed = python_test_symbols(before) - python_test_symbols(path.read_text(encoding="utf-8"))
        for symbol in sorted(removed):
            target = f"{rel}#{symbol}"
    ...
    if before_state_raw is not None and (root / active_rel).is_file():
        try:
            violations.extend(state_regressions(json.loads(before_state_raw), load_json(root / active_rel), transitions, root))
    ...
    if manifest_rel in changed and (root / manifest_rel).is_file():
        ...
        if before_manifest is not None:
            ...
            after_permissions, after_components = manifest_contract((root / manifest_rel).read_text(encoding="utf-8"))
