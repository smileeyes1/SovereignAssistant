...
class TemplateCodingWorker:
    """Vendor-free baseline worker used to prove orchestration semantics."""
    ...
    def build(self, plan: AppPlan) -> dict[str, str]:
        ...
        html = (
            ...
            f"<title>{escaped}</title><main><h1>{escaped}</h1>"
            "<p>تم إنشاء هذا المشروع عبر Ω Forge.</p></main></html>"
        )
        return {"index.html": html}

    def repair(self, plan: AppPlan, files: dict[str, str], failures: tuple[str, ...]) -> dict[str, str]:
        # The deterministic baseline repairs by rebuilding from the source plan.
