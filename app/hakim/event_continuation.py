...
This module removes polling from the decision core. External systems may still
use schedulers as a fallback, but continuation is triggered by concrete events
and remains constrained by safety, reversibility, authority, and deduplication.
"""
...
class EventDrivenContinuation:
    """React immediately to events and execute the highest-value allowed action.

    The engine is deliberately transport-agnostic: GitHub webhooks, runtime
    events, queues, local processes, or future MCP/A2A adapters can all feed the
    same ContinuationEvent contract.
    """
    ...
    def handle(self, event: ContinuationEvent) -> ContinuationDecision:
        ...
        candidates = sorted(
            (c for c in self.action_source(event) if c.executable),
        ...
        if not candidates:
            ...
            decision = ContinuationDecision(
                ...
                None,
                "blocked",
                "no safe reversible authorized ready action",
            )
            self.decisions.append(decision)
            return decision
        ...
        decision = ContinuationDecision(
            ...
            selected.name,
            "executed",
            "highest-value eligible action executed",
        )
        self.decisions.append(decision)
        return decision
...
@dataclass
class EventRouter:
    """Small in-process event bus for immediate chained continuation."""
