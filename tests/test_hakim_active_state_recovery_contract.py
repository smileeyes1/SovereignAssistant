...
def test_active_state_restores_value_gates_and_physical_preflight_gate():
    """Stable contract id retained; 2.3 records the verified max-value restore contract."""
    state = load(STATE)
    assert state["max_value_intent_rule"] == "governance/HAKIM_MAX_VALUE_INTENT_RULE_AR.md"
    assert state["max_value_intent_rule_enforcement"] == "ACTIVE_VERIFIED__RESTORE_AND_PROMOTION_GATE_ENFORCED_ON_MAIN"
    assert "LOAD_AND_ENFORCE_MAX_VALUE_INTENT_RULE" in state["resume_rule"]
    assert state["promotion_evidence"]["max_value_intent_rule_v1_1_restore_contract_main"] == "PASS"
    assert state["promotion_evidence"]["max_value_intent_rule_v1_1_governance_main"] == "PASS"
    assert state["promotion_evidence"]["max_value_intent_rule_v1_1_continuity_main"] == "PASS"
    assert state["promotion_evidence"]["max_value_intent_rule_v1_1_reality_main"] == "PASS"
    assert "max_value_intent_rule_v1_1_restored_governing_contract" in state["proven_success"]
