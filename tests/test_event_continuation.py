...
from app.hakim.event_continuation import (
    ActionCandidate,
    ContinuationEvent,
    EventDrivenContinuation,
    EventRouter,
    EventType,
)
...
def test_blocks_when_only_unsafe_or_unauthorized_actions_exist():
    engine = EventDrivenContinuation(
        lambda event: [
            ActionCandidate("irreversible", 50, True, False, True),
            ActionCandidate("unauthorized", 40, True, True, False),
        ],
...
def test_duplicate_event_is_idempotent():
    executed = []
    engine = EventDrivenContinuation(
        lambda event: [ActionCandidate("next", 1, True, True, True)],
        lambda action, event: executed.append(event.event_id),
    )
...
def test_failed_execution_can_be_retried_with_same_event():
    ...
    engine = EventDrivenContinuation(
        lambda event: [ActionCandidate("retryable", 1, True, True, True)],
        execute,
    )
...
def test_router_dispatches_immediately_to_subscribers():
    ...
    engine = EventDrivenContinuation(
        ...
        lambda action, event: seen.append((action.name, event.subject)),
    )
    router = EventRouter()
