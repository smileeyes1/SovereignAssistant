...
def test_secure_bridge_override_is_fail_closed_and_respects_phone_constraint():
    ...
    assert policy["phone_execution"] == "TERMUX_WIRELESS_ADB_LOCAL_GOVERNED"
    assert policy["adb_role"] == "GOVERNING_LOCAL_FIELD_PATH_WITH_ANDROID_REQUIRED_LOCAL_PAIRING"
...
def test_termux_bootstrap_is_governing_local_path_but_remains_fail_closed():
    ...
    assert "بقي حاجز أندرويد الوحيد: الاقتران الأول بالتصحيح اللاسلكي" in boot
    assert not (ROOT / ".github/workflows/hakim-phone-relay.yml").exists()
    runtime = load_json("governance/HAKIM_RUNTIME_POLICY_v2.json")
    assert runtime["bridges"]["preferred"][0] == "TERMUX_WIRELESS_ADB_LOCAL"
