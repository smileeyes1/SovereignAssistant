...
def test_phone_interface_is_active_verified_single_hakim_without_field_promotion():
    ...
    assert promotion["feature_merged_main"] == "f054ee0900e8d907b0ce0166b71ab64c83e0d225"
    assert promotion["identity_activation_main"] == "93c1abe9d7a7bfb75ec974b6e8adf91db2cfd4bf"
    assert promotion["verified_repository_truth_main"] == promotion["identity_activation_main"]
    ...
    assert overlay["governance_verified_main"] == promotion["verified_repository_truth_main"]
    assert overlay["freshness_observed_main"] == promotion["verified_repository_truth_main"]
    assert overlay["freshness_state_refresh_source_main"] == promotion["verified_repository_truth_main"]
    assert overlay["promotion_lineage_additions"]["canonical_phone_interface_v1"] == promotion["identity_activation_main"]
    assert overlay["promotion_lineage_additions"]["android_airplane_recovery_resilience_v1"] == promotion["feature_merged_main"]
